...
age_null_count = sum(1 for age in age_null if age == True)
...
for age in age_null:
    if age == False:
        not_null.append(True)
    else:
        not_null.append(False)

correct_ages = titanic_survival["age"].loc[not_null]
correct_mean_age = sum(correct_ages) / len(correct_ages)
...
for pclass in passenger_classes:
    rows = (titanic_survival["pclass"] == pclass)
    mean_fare = titanic_survival["fare"].loc[rows].mean()
...
age_labels = titanic_survival.apply(age_cat, axis=1)
