...
ax.plot(day_numbers, snail_crawl_length)
ax.set_xlabel("Days")
ax.set_ylabel("Snail Crawl Length")
plt.show()

fig = plt.figure(figsize=(8,8))
ax = fig.add_subplot(1,1,1)
ax.plot(day_numbers, cars_in_parking_lot)
ax.set_xlabel("Days")
ax.set_ylabel("Cars in Parking Lot")
plt.show()
...
# Results from our survey on how many cigarettes people smoke per daysurvey_responses = ["none", "some", "a lot", "none", "a few", "none", "none"]
...
for i,row in enumerate(savings):
    if gender[i] == "male":
        male_vals.append(row)
    else:
        female_vals.append(row)

male_savings = sum(male_vals) / float(len(male_vals))
female_savings = sum(female_vals) / float(len(female_vals))
