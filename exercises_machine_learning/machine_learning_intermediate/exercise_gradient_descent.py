...
costs = [cost(0,theta1,pga.distance,pga.accuracy) for theta1 in theta1s]
...
i_vals = range(len(theta0s))
j_vals = range(len(theta1s))
for i in i_vals:
    for j in j_vals:
...
results = gradient_descent(pga.distance,pga.accuracy,alpha=0.01)
plt.plot(range(len(results['costs'])), results['costs'])
plt.show()