...
min_overall = income["median_income"].min()
min_large = income["median_income"].loc[income["pop_over_25"] > 500000].min()
lowest_income_county = income["county"].loc[income["median_income"] == min_overall].values[0]
lowest_income_high_pop_county = income["county"].loc[income["median_income"] == min_large].values[0]

# Alternative, using idxmin()
...
over_18_count = sum([x >= 0.18 for x in large_sample])
...
significance_value = sum([x >= 0.675 for x in mean_ratios]) / float(len(mean_ratios))
