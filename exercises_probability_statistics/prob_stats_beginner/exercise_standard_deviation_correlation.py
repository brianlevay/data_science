...
point_variance = sum([(x-pts_mean)**2 for x in list(nba_stats["pts"])]) / len(nba_stats["pts"])
...
def stdev(list_vals):
    mean_val = sum(list_vals)/len(list_vals)
    sum_squares = sum([(x-mean_val)**2 for x in list_vals])
    stdev = (sum_squares / float(len(list_vals))) ** (1/2)
    return stdev

mp_dev = stdev(list(nba_stats["mp"]))
ast_dev = stdev(list(nba_stats["ast"]))
...
probs2 = norm.pdf(points2, 0, 2)
plt.plot(points2, probs2)
plt.show()
...
wing_length_mean = sum(wing_lengths) / len(wing_lengths)
wing_length_std = stdev(wing_lengths)
wing_length_dist = [abs( (x-wing_length_mean)/wing_length_std ) for x in wing_lengths]

def percent_in_std(vals, std):
    count = 0
    for num in vals:
        if num <= std:
            count += 1
    return count/float(len(vals))

within_one_percentage = percent_in_std(wing_length_dist, 1)
within_two_percentage = percent_in_std(wing_length_dist, 2)
within_three_percentage = percent_in_std(wing_length_dist, 3)
...
cov_stl_pf = covariance(list(nba_stats["stl"]), list(nba_stats["pf"]))
cov_fta_pts = covariance(list(nba_stats["fta"]), list(nba_stats["pts"]))
...
maxvar_fta_blk = nba_stats["fta"].std() * nba_stats["blk"].std()
r_fta_blk = cov_fta_blk / maxvar_fta_blk
print(r_fta_blk)

cov_ast_stl = cov(nba_stats["ast"], nba_stats["stl"])[0,1]
maxvar_ast_stl = nba_stats["ast"].std() * nba_stats["stl"].std()
r_ast_stl = cov_ast_stl / maxvar_ast_stl
print(r_ast_stl)